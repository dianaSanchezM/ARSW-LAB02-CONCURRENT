...
public class HostBlackListsValidator {
    ...
    public List<Integer> checkHost(String ipaddress, int N){
        ...
        for (int j=0; j<N; j++){
            if (j==N-1){
                threadList.add(new SearchThread(j*numServers, skds.getRegisteredServersCount(), ipaddress,ocurrencesCount));
            }else{
                threadList.add(new SearchThread(j*numServers, (j+1)*numServers, ipaddress, ocurrencesCount));
            }
        }
